...
$titulosColumnas = array('ID','NOMBRE', 'USUARIO', 'CORREO','ESTADO');
...
foreach( $this->_view->report_users as $user) {
     $objPHPExcel->setActiveSheetIndex(0)
         ->setCellValue('A'.$i, $user['iduser'])
         ->setCellValue('B'.$i, $user['nombre'])
         ->setCellValue('C'.$i, $user['usuario'])
         ->setCellValue('D'.$i, $user['correo'])
...
$objPHPExcel->getActiveSheet()->setTitle('Informe de Usuarios');
